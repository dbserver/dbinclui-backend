...
describe("GetCategoriesAndContentController", () => {
  ...
  it("Should return 200 OK if get success in requisition", async () => {
    ...
    const { body } = await request(app.getExpress).get(`/guides/categoriesAndContent/${guide._id}`).expect(200);

    expect(body.data.categories[0].digitalContents[0])
    .toHaveProperty("_id", digitalContent._id.toString());
  });

  it(`Should return an empty object if guide with content not exists`, async () => {
    const { body } = await request(app.getExpress)
    .get("/guides/categoriesAndContent/invalidGuideWithContentID")
