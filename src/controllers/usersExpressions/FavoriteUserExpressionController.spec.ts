...
describe("FavoriteUserExpressionController", () => {
  let express: App;
  ...
  it("Should return status 403 and a message format token invalid", async () => {
    ...
    await request(express.getExpress)
      .patch("/usersExpressions/favorite/1231231")
  ...
  it("Should return a status 400 if expressionID does not exist", async () => {
    ...
    await request(express.getExpress)
      .patch("/usersExpressions/favorite/6382b82611a630eb7bf0fb7e")
  ...
  it("Should delete and return an EntityFavorited", async () => {
    verifyIdTokenMock.mockReturnValue({
      uid: "123",
    });
    ...
    await request(express.getExpress)
      .patch(`/usersExpressions/favorite/${expression._id}`)
      .set("Authorization", `Bearer ${token}`)
      .expect(200)
      .then((data) => {
        expect(data.body.data._id).toBe(expression._id.toString());
        expect(data.body.data.expression).toBe("Expressão de test");
        expect(data.body.data.author).not.toBeNull();
        expect(data.body.data.favorite).toBeTruthy();
      });
  });
});
