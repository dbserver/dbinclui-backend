...
describe("GetByIdDigitalContentController", () => {
  ...
  it("Should return a status of 400 if the digital content does not exist", async () => {
    const response = await request(app.getExpress).get(`/digital-contents/6203fd2780cb7f271145302f`)
    expect(400);
  ...
  it("Should return a status of 400 if id is invalid", async () => {
    ...
    const response = await request(app.getExpress).get(`/digital-contents/${firstDigitalContent._id}48518`)
    expect(400);
    const {message} = response.body
