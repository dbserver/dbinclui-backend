...
describe("GetByCategoryIdDigitalContentController", () => {
  ...
  it("Should return an empty array", async () => {
    const response = await request(app.getExpress).get(`/digital-contents/category/6203f8e880cb7f271145311f`)
  ...
  it("Should return 1 digital contents", async () => {
    ...
    const response = await request(app.getExpress).get(`/digital-contents/category/${category._id}`)
    const {data} = response.body
